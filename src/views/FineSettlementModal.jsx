...
import { X, AlertCircle, CheckCircle, CreditCard, Receipt } from 'lucide-react';
                                ...
                                {fines.map(fine => (
                                    <div key={fine.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-gray-50 dark:bg-slate-700 rounded-xl border border-gray-100 dark:border-slate-600 hover:border-red-200 transition-colors">
                                        ...
                                        <div className="flex items-center gap-4 mt-3 sm:mt-0 w-full sm:w-auto">
                                            <span className="font-bold text-red-600 dark:text-red-400 text-lg">₱{parseFloat(fine.penalty_amount).toFixed(2)}</span>
                                            <button
                                                onClick={() => handlePay(fine.id, fine.penalty_amount)}
                                                className="flex-1 sm:flex-none px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-bold flex items-center justify-center gap-1 transition-colors shadow-md shadow-red-200"
                                            >
